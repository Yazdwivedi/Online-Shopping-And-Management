...
if(isset($_POST["rem"])){
	$id=$_POST["id"];
	$result=$conn->query("delete from info where id='{$id}'");
...
<div class="collapse navbar-collapse justify-content-end" id="navbarSupportedContent">
  ...
  <table>
    		<?php
		$result=$conn->query("select info.id,info.name,info.number,info.quantity,info.address,info.landmark,info.city,info.state,info.pincode,item.title from info inner join item on item.id=info.i_id inner join seller on item.a_id=seller.id where seller.id={$_SESSION["id"]}");
		...
		?>
          <tr>
          <td style="text-decoration:underline;"><b>Item</b></td>
          <td class="right" style="text-decoration:underline;"><b>Quantity</b></td>
          <td style="text-decoration:underline;"><b>Address</b></td>
          <td class="left" style="text-decoration:underline;"><b>Customer Info</b></td>
          <td class="right" style="text-decoration:underline;"><b>Remove</b></td>
        </tr>
        ...
        <tr>
          ...
          <td style="padding-left:4.5%;"><?php echo $row->quantity;?></td>
          <td><?php echo $row->address.",".$row->city.",".$row->state.",".$row->pincode." near ".$row->landmark;?></td>
          <td><?php echo"Name:".$row->name."<br>Phone:".$row->number;?></td>
		  <form method="post">
		  <input type="hidden" name="id" value="<?php echo$row->id;?>">
          <td style="padding-left:2%;"><button type="submit" name="rem" class="btn btn-danger">Remove</button></td>
		  </form>
        </tr>
		  ...
		  ?>
		  <h3 style="margin-left:38%;">No Items to be Transported</h3><?php
	  }
	  ?>
