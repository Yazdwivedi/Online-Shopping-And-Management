...
<div class="collapse navbar-collapse justify-content-end" id="navbarSupportedContent">
  ...
  <div class="jumbotron top">
      <table>
        <tr>
          <td><b>Item</b></td>
          <td class="left"><b>Quantity Remaining</b></td>
          <td class="right"><b>Status</b></td>
          <td class="right"><b>Update</b></td>
		  <td class="right"><b>Change Image</b></td>
        </tr>
		<?php  
		
		$result=$conn->query("select item.quantity,item.title,item.id,item.int_quantity from item inner join seller on item.a_id=seller.id where item.a_id={$_SESSION["id"]} order by item.quantity desc");
        ...
        <tr>
          ...
          <td class="left" style="padding-left:4.5%;"><?php echo$row->quantity;?></td>
          <td class="right"><?php 
		  if($row->quantity<3 and $row->quantity>0){
			  if($row->int_quantity==$row->quantity){
				  echo "Unsold";
				  
			  }
			  else{
				  echo "Sold ".($row->int_quantity-$row->quantity)." items";
			  }
		  }
			...
			<form method="post" enctype="multipart/form-data">
				...
				<td class="right"><button type="submit" class="btn btn-primary" name="ups">Update</button></td>
			</form>
			<form method="post" enctype="multipart/form-data">
			<td><input type="file" name="image"><br>
			<input type="hidden" value="<?php echo$row->id?>" name="id">
			<button style="margin-top:2%;" type="submit" class="btn btn-primary" name="img">Change</button></td>
			</form>
        </tr>
		<?php }?>
      </table>
