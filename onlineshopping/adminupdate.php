...
if(isset($_POST["upd"])){
	...
	$result=$conn->query("update item set title='{$name}',description='{$des}',quantity='{$q}',type='{$t}',discount='{$d}',price='{$p}',int_quantity='{$q}' where id={$_SESSION["itemid"]}");
...
<div class="collapse navbar-collapse justify-content-end" id="navbarSupportedContent">
  ...
  <div class="jumbotron top">
    <h3 style="margin-left:40%;">Update The Items</h3><br>
	<?php 
		$result=$conn->query("select * from item where id={$_SESSION["itemid"]}");
		...
		?>
            <form method="post" enctype="multipart/form-data">
            <label>Item Name</label>
            <input class="form-control" type="text" name="name" value="<?php echo$row->title;?>">
            <label>Description</label>
            <textarea class="form-control" name="des" rows="8" cols="50"><?php echo$row->description;?></textarea>
            <label>Quantity</label>
            <select class="form-control" name="q">
              <option selected hidden value="<?php echo$row->quantity;?>"><?php echo$row->quantity;?></option>
              <option value="1">1</option>
              <option value="2">2</option>
              <option value="3">3</option>
            </select>
			<label>Price</label>
			<input type="number" class="form-control" name="p" value="<?php echo$row->price;?>">
            <label>Type</label>
            <select class="form-control" name="t" >
              <option selected hidden value="<?php echo$row->type;?>"><?php if($row->type==1){
				  echo "Electronics";
			  }
			  elseif($row->type==2){
				  echo"Books";
            ...
            <label>Discount</label>
            <select class="form-control" name="d">
              <option selected hidden value="<?php echo$row->discount;?>"><?php if($row->discount==1){
				  echo "75%";
			  }
            ...
            <button type="submit" class="btn btn-primary" name="upd" style="margin-top:2%;margin-left:1.5%;">Update</button>
          </form>
